...
function createBooking(data) {
  const {
    ...
    about_child, wishes, source, contact_handle
  } = data;
  ...
  const tx = db.transaction(() => {
    ...
    `).run(stream_id, child_name.trim(), age, phone, (parent_name || '').trim(), gender,
      ...
      (about_child || '').trim(), (wishes || '').trim(), src, (contact_handle || '').trim(), basePrice);
...
module.exports = { createBooking, updateBooking, LEAD_STATUSES, SOURCES, GENDER_LIMIT };
